...
export default function Products({ items }: ProductsProps) {
    return (
        <section>
            <h1 className="text-4xl text-center font-bold mb-6">Explore Our wide range of Products</h1>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-8">
                {items.map((item, index) => (
                    <div key={index} className="m-1 p-2 bg-neutral-100 rounded-xl border border-neutral-300 max-w-96">
                        <img src={item.image} alt={item.name} className="w-full object-cover rounded-lg aspect-[1.25]" />

                        <div className="bottom flex justify-between items-center m-2">
                            <div className="left">
                                <h2 className="text-md font-bold mt-2">{item.name}</h2>
                                <div className="flex items-center">
                                    <p className="text-gray-800 text-md mr-2">₹{item.price}</p>
